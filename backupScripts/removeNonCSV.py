  ...
  if(fileName.startswith(inputPrefix)):
    ...
    with open(fileName) as inFile:
      ...
      for currLine in lines:
        if ',' in currLine:
          #pdb.set_trace()
          #There's gotta be a way for this to be cleaned up
          #if (not (( "cycle" in currLine) or ("time" in currLine) or ("otal" in currLine) ) ):
          if (not (( "cycle" in currLine) or ("otal" in currLine) ) ):
