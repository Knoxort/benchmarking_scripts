...
eprList="5 10 15 20 25 30";	#Elements per Rank List	

rankList="27 64 125"; # 1331 1728";	#Number of Ranks	
...
do
	...
	do
		...
		#Start actual job file
	        ...
	        echo "#SBATCH --account=lam" >> $outputJobFile                          	      
	        echo "#SBATCH --qos=lam-b" >> $outputJobFile                          	      
		echo "##SBATCH -p pbatch" >> $outputJobFile                  #For TST18, only 1 node/16 cores for quartz
	        ...
	        echo "" >> $outputJobFile
		echo "ml intel/2020 openmpi/4.0.3 silo visit hdf5 boost/1.71" >> $outputJobFile
	        echo "" >> $outputJobFile
